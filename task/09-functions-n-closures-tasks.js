...
function logger(func, logFunc) {
    return function(){
        ...
        for(var argument in arguments)
            funcStr += JSON.stringify(arguments[argument]) + (argument < arguments.length-1 ? "," : "");
        funcStr += ')';
...
function partialUsingArguments(fn) {
    ...
    if(args.length>1) {
        var nargs = [];
        for (var i = 1; i < args.length; i++) nargs.push(args[i]);
        return function () {
            var cargs = nargs.slice();
            for (var i = 1; i < arguments.length; i++) cargs.push(args[i]);
            console.log(cargs);
            return fn.call(this, cargs);
        }
    }
    else return fn;
}
